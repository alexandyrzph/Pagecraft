    ...
    <>
      <Card title="Profile">
        <form onSubmit={saveName} className="space-y-3">
          ...
          <Button
            ...
            leadingIcon={nameOk ? <Check size={15} /> : undefined}
          >
            Save
          </Button>
        </form>
      ...
      <Card title="Password" desc="Changing your password signs out your other sessions.">
        <form onSubmit={savePw} className="space-y-3">
          ...
          />
          {pwMsg && (
            <p className={pwMsg.ok ? "text-xs text-emerald-600" : "text-xs text-danger-600"}>
              {pwMsg.text}
            </p>
          )}
          <Button type="submit" variant="neutral" isLoading={pwBusy}>
            Update password
          </Button>
        </form>
      </Card>
