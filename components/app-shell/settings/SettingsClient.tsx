      ...
      <div className="mt-6 flex gap-1 border-b border-border">
        {tabs.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 text-sm font-medium capitalize ${tab === t ? "border-b-2 border-brand-600 text-brand-700" : "text-fg-muted hover:text-fg"}`}
          >
            {t}
          </button>
        ))}
...
function Members() {
  ...
  async function remove(m: Member) {
    const ok = await confirm({
      title: "Remove member?",
      message: `${m.email} will lose access to this workspace.`,
      confirmLabel: "Remove",
      destructive: true,
    });
    if (!ok) return;
      ...
      {members.map((m) => (
        <div key={m.membershipId} className="flex items-center gap-3 px-4 py-3">
          ...
          <Select
            ...
            selectedKey={m.role}
            onSelectionChange={(k) => changeRole(m, String(k))}
          />
          <Button
    ...
    <div className="space-y-5">
      <form onSubmit={create} className="flex flex-wrap items-end gap-2">
        ...
        <Select
          ...
          className="w-32"
          items={["VIEWER", "EDITOR", "ADMIN"].map((r) => ({ id: r, label: r }))}
          selectedKey={role}
          onSelectionChange={(k) => setRole(String(k))}
        />
        <Button type="submit" variant="neutral" isLoading={busy}>
          Invite
        </Button>
      </form>
      {err && <p className="text-xs text-danger-600">{err}</p>}
