...
export type DnsRecord = { record: string; type: string; value: string };
export type Domain = {
  id: string;
  hostname: string;
...
export function useDomainsManager() {
  ...
  return { domains, hostname, setHostname, adding, addErr, verifyingId, add, verify, remove };
}
...
export function DomainsEmptyState({ canManage }: { canManage: boolean }) {
  return (
