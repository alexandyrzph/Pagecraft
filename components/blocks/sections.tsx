        ...
        <div className="mt-2">
          ...
          >
            <Editable
              ...
              value={buttonText}
              editable={editable}
              onCommit={(v) => setProp("buttonText", v)}
            />
          </a>
        </div>
      </div>
...
export function FeatureGridBlock({
  ...
  const {
    ...
    items = [] as Feature[],
    columns = 3,
  } = block.props as { title?: string; subtitle?: string; items?: Feature[]; columns?: number };
  return (
          ...
          {(items as Feature[]).map((f, i) => (
            ...
            >
              ...
              <Editable
                ...
                value={f.title}
                editable={editable}
                onCommit={(v) => setProp(`items.${i}.title`, v)}
              ...
              <Editable
                ...
                value={f.text}
                editable={editable}
                onCommit={(v) => setProp(`items.${i}.text`, v)}
...
export function PricingBlock({ block, editable, style, className, id, setProp }: BlockRenderProps) {
  const {
    ...
    items = [] as Plan[],
  } = block.props as { title?: string; subtitle?: string; items?: Plan[] };
  return (
    <section id={id} className={cn("w-full", className)} style={style}>
      <div className="mx-auto max-w-6xl px-6 py-20">
        <div className="mx-auto mb-14 max-w-2xl text-center">
          ...
          {(items as Plan[]).map((p, i) => (
            <div
              ...
              className={cn(
                ...
                p.featured
                  ? "border-indigo-500 bg-slate-900 text-white shadow-xl"
            ...
            >
              <Editable
                ...
                value={p.name}
                editable={editable}
                onCommit={(v) => setProp(`items.${i}.name`, v)}
              ...
              <div className="mt-4 flex items-baseline gap-1">
                <Editable
                  ...
                  value={p.price}
                  editable={editable}
                  onCommit={(v) => setProp(`items.${i}.price`, v)}
              ...
              <ul
                className={cn(
                  ...
                  p.featured ? "text-slate-200" : "text-slate-600",
                )}
              >
                {(p.features ?? "")
                  .split("\n")
              ...
              >
                <Editable
                  ...
                  value={p.buttonText}
                  editable={editable}
                  onCommit={(v) => setProp(`items.${i}.buttonText`, v)}
                />
              </a>
            </div>
          ))}
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <DynamicIcon
              ...
              style={{
                ...
                fill: i < Number(rating) ? "#f59e0b" : "transparent",
              }}
            />
          ))}
        </div>
...
export function StatsBlock({ block, editable, style, className, id, setProp }: BlockRenderProps) {
  ...
  return (
    <section id={id} className={cn("w-full", className)} style={style}>
      <div
        className="mx-auto grid max-w-5xl gap-8 px-6 py-16"
        style={{
          gridTemplateColumns: `repeat(${Math.min((items as Stat[]).length || 1, 4)}, minmax(0, 1fr))`,
        }}
      >
        {(items as Stat[]).map((s, i) => (
          <div key={i} className="text-center">
            <Editable
              ...
              value={s.value}
              editable={editable}
              onCommit={(v) => setProp(`items.${i}.value`, v)}
              style={{ color: "var(--pc-brand, #6366f1)" }}
            ...
            <Editable
              ...
              value={s.label}
              editable={editable}
              onCommit={(v) => setProp(`items.${i}.label`, v)}
...
export function FooterBlock({ block, editable, style, className, id, setProp }: BlockRenderProps) {
  ...
  } = block.props as { brand?: string; tagline?: string; links?: string[]; copyright?: string };
  return (
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-6 py-14">
        ...
        <div className="border-t border-white/10 pt-6">
          <Editable
            ...
            value={copyright}
            editable={editable}
            onCommit={(v) => setProp("copyright", v)}
