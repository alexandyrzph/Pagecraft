    ...
    <label className="block">
      <span className="mb-1 flex items-center justify-between">
        <span className="flex items-center gap-1 text-[11px] font-medium text-zinc-500">
          {label}
          {active && (
            <span className="h-1 w-1 rounded-full bg-indigo-400" title="Set for this breakpoint" />
          )}
        </span>
        {active && (
          <button
...
function SpacingControl({
  ...
  label: string;
  keys: [keyof StyleProps, keyof StyleProps, keyof StyleProps, keyof StyleProps];
}) {
      ...
      <div className="mb-1 flex items-center justify-between">
        ...
        <button
          ...
          className={cn(
            ...
            linked ? "text-indigo-500" : "text-zinc-300 hover:text-zinc-500",
          )}
        >
          <Link2 size={12} />
        </button>
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {cells.map((c, i) => (
            <div key={i}>
              ...
              <span className="mt-0.5 block text-center text-[9px] font-semibold text-zinc-400">
                {["T", "R", "B", "L"][i]}
              </span>
            </div>
          ))}
