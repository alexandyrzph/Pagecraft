...
import { ColorInput, Field, Segmented, SelectInput, UnitInput } from "./controls";
...
function PanelSection({
  ...
  return (
    <div className="border-b border-zinc-200/70 px-3 py-3 last:border-b-0">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between text-[11px] font-semibold uppercase tracking-wider text-zinc-500 transition-colors hover:text-zinc-700"
      >
    ...
    <div className="space-y-3">
      {colors.length === 0 && (
        <p className="text-[11px] leading-snug text-zinc-400">
          Define reusable colors once. Pick them in any color field — editing a color style updates
      ...
      {colors.map((c) => (
        <div key={c.id} className="rounded-lg border border-zinc-200 bg-white p-2">
          ...
          <ColorInput
            value={c.value}
            onChange={(v) => updateColor(c.id, { value: v })}
            hideTokens
          />
        </div>
      ))}
...
function TextStyleRow({ ts }: { ts: TextStyle }) {
  ...
  const previewStyle: React.CSSProperties = {
    ...
    fontWeight: ts.props.fontWeight as React.CSSProperties["fontWeight"],
    letterSpacing: ts.props.letterSpacing,
    textTransform: ts.props.textTransform as React.CSSProperties["textTransform"],
    ...
    <div className="rounded-lg border border-zinc-200 bg-white">
      <div className="flex items-center gap-1.5 p-2">
        ...
        <input
          ...
          className="min-w-0 flex-1 rounded-md border border-transparent px-1.5 py-1 text-[13px] font-medium text-zinc-700 outline-none hover:border-zinc-200 focus:border-indigo-300"
        />
        <span className="max-w-[70px] truncate text-zinc-400" style={previewStyle}>
          Ag
        </span>
        <button
          onClick={() => removeTextStyle(ts.id)}
            ...
            <div className="space-y-2.5 border-t border-zinc-100 p-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Field label="Letter spacing">
                  <UnitInput
                    value={tsField(ts, "letterSpacing")}
              ...
              <Field label="Color">
                <ColorInput value={tsField(ts, "color")} onChange={set("color")} />
              </Field>
              <div className="grid grid-cols-2 gap-2">
                <Field label="Align">
                  <Segmented
                    value={tsField(ts, "textAlign")}
                ...
                <Field label="Transform">
                  <SelectInput
                    value={tsField(ts, "textTransform")}
...
function TextStyles() {
  ...
  return (
    <div className="space-y-2.5">
      {textStyles.length === 0 && (
        <p className="text-[11px] leading-snug text-zinc-400">
          Save typography presets (headings, body, captions). Apply them to any text block from the
          inspector — edit once, update all.
        </p>
      )}
      {textStyles.map((ts) => (
      ...
      <button
        onClick={() =>
          addTextStyle(`Text style ${textStyles.length + 1}`, {
            fontSize: "16px",
            lineHeight: "1.5",
          })
        }
  ...
  return (
    <div className="pb-4">
      <PanelSection title="Theme" icon={<PaletteIcon size={12} />}>
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              onClick={() =>
                setTheme({ brand: PRESETS[Math.floor(Math.random() * PRESETS.length)] })
              }
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {PRESETS.map((c) => (
              <button
                ...
                className={cn(
                  "h-6 w-6 rounded-full ring-2 ring-offset-2 transition-transform hover:scale-110",
                  theme.brand === c ? "ring-zinc-900" : "ring-transparent",
                )}
      ...
      <p className="mx-3 mt-3 rounded-lg bg-zinc-100 px-3 py-2 text-[11px] leading-relaxed text-zinc-500">
        Color &amp; text styles are{" "}
        <span className="font-medium text-zinc-600">shared across every page</span> on the site.
      </p>
    </div>
  );
}
